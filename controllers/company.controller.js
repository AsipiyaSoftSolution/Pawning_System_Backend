  ...
  try {
    ...
    if (accountType && accountType === "cashier") {
      ...
      const [cashierAccount] = await pool.query(
        "INSERT INTO accounting_accounts (Account_Name, Account_Type, Created_At,Type,Group_Of_Type,User_idUser,Cashier_idCashier,Branch_idBranch,Account_Balance,Status,Account_Code,isCashier) VALUES (?,?,NOW(),?,?,?,?,?,?,?,?,?)",
        [
...
export const createBranch = async (req, res, next) => {
  try {
    ...
    const [headOfficeAccount] = await pool.query(
      "INSERT INTO accounting_accounts (Account_Name, Account_Type, Created_At, Type, Group_Of_Type, Account_Balance, Status, Branch_idBranch, User_idUser,Account_Code) VALUES (?, ?, NOW(), ?, ?, ?, ?, ?, ?,?)",
      [
